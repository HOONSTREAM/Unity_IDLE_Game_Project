...
public class Spawner : MonoBehaviour
{
    ...
    private void Back_To_MainGame_Map()
    {
        ...
        Maps[4].gameObject.SetActive(false);
    ...
    public void OnDungeon(int Value)
    {
        ...
        if(Value == 0) // 다이아몬드 던전
        {
            ...
            coroutine = StartCoroutine(SpawnCoroutine(30, -1, (Stage_Manager.Dungeon_Level + 1) * Utils.DIA_DUNGEON_MULTIPLE_HARD));
        ...
        if (Value == 2) // 미스릴 던전
        {
            Stop_Coroutine_And_Delete_Monster();
            Base_Manager.Pool.Clear_Pool(); // 풀링객체 초기화
            StartCoroutine(Enhancement_BossSetCoroutine());
        }
    ...
    IEnumerator BossSetCoroutine(int Player_Stage = 0)
    {
        ...
        else
        {
            ...
            {
                // 풀링이 생성될때의 기능을 구현한다.
                value.GetComponent<Monster>().Init((Stage_Manager.Dungeon_Level + 1) * Utils.GOLD_DUNGEON_MULTIPLE_HARD); 
            });
    ...
    IEnumerator Tier_BossSetCoroutine()
    {
        ...
        int difficulty = Mathf.RoundToInt(Mathf.Pow(dungeon_level, 3.0f) * Utils.TIER_DUNGEON_FIRST_HARD);

        if(difficulty == 0)
        {
            difficulty = Utils.TIER_DUNGEON_FIRST_HARD;
        }
    ...
    IEnumerator Enhancement_BossSetCoroutine()
    {
        yield return new WaitForSeconds(2.0f);
        Monster monster = null;

        int dungeon_level = Stage_Manager.Dungeon_Level;
        int difficulty = Utils.ENHANCEMENT_DUNGEON_FIRST_HARD;
        ...
        var go = Base_Manager.Pool.Pooling_OBJ("Enhancement_Dungeon").Get((value) =>
        {
            // 풀링이 생성될때의 기능을 구현한다.
            value.GetComponent<Monster>().Init(Stage_Manager.Dungeon_Level * difficulty);
        });
